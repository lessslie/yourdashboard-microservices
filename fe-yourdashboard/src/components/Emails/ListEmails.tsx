...
import { Layout, Button, List, Skeleton, Pagination, Card, message } from "antd";
...
const ListEmails = () => {
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      ...
      try {
        setInitLoading(true);
        console.log(`📧 Fetching emails for cuenta Gmail: ${selectedCuentaGmailId}, página: ${page}`);
        
        const emailsResponse = await getEmails(token, selectedCuentaGmailId, page, limit);
  ...
  if (loadingProfile) {
    return (
      <Content style={{ padding: "0 48px", textAlign: "center", paddingTop: "50px" }}>
        <Skeleton active />
