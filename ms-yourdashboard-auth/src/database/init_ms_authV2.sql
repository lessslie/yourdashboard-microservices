...
-- ✅ VERSIÓN MIGRADA UUID - Actualizada para ms-auth
...
CREATE TABLE usuarios_principales (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- ✅ SERIAL → UUID
...
CREATE TABLE cuentas_gmail_asociadas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- ✅ SERIAL → UUID
  usuario_principal_id UUID NOT NULL, -- ✅ INTEGER → UUID
...
CREATE TABLE emails_sincronizados (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- ✅ SERIAL → UUID
  cuenta_gmail_id UUID NOT NULL, -- ✅ INTEGER → UUID
...
CREATE TABLE audit_eliminaciones (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), -- ✅ SERIAL → UUID
  tabla VARCHAR(50),
  registro_id UUID, -- ✅ INTEGER → UUID (para referenciar cualquier tabla con UUID)
...
---------------------------------------------------
-- ✅ FUNCIÓN CORREGIDA: obtener_cuentas_gmail_usuario
-- Cambio de INTEGER → UUID para p_usuario_id
---------------------------------------------------
CREATE OR REPLACE FUNCTION obtener_cuentas_gmail_usuario(p_usuario_id UUID) -- ✅ INTEGER → UUID
RETURNS TABLE (
    cuenta_id UUID, -- ✅ INTEGER → UUID
    email_gmail VARCHAR,
...
------------------------------------------------------
-- ✅ FUNCIÓN CORREGIDA: eliminar_cuenta_gmail_segura
-- Cambio de INTEGER → UUID para p_cuenta_id
------------------------------------------------------
CREATE OR REPLACE FUNCTION eliminar_cuenta_gmail_segura(p_cuenta_id UUID) -- ✅ INTEGER → UUID
RETURNS JSON AS $$
DECLARE
    cuenta_info RECORD;
...
--------------------------------------------------------
-- ✅ FUNCIÓN CORREGIDA: obtener_stats_emails_cuenta
-- Cambio de INTEGER → UUID para p_cuenta_id
--------------------------------------------------------
CREATE OR REPLACE FUNCTION obtener_stats_emails_cuenta(p_cuenta_id UUID) -- ✅ INTEGER → UUID
RETURNS TABLE (
    total_emails BIGINT,
...
---------------------------------------------------------------------------------
-- ✅ FUNCIÓN CORREGIDA: obtener_reporte_emails_antiguos
-- Cambio de tipos INTEGER → UUID
---------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION obtener_reporte_emails_antiguos(
    p_dias_antiguedad INTEGER DEFAULT 365
)
RETURNS TABLE (
    cuenta_gmail_id UUID, -- ✅ INTEGER → UUID
    email_gmail VARCHAR,
...
-- =====================================
-- 📋 NOTA: WHATSAPP SEPARADO
-- =====================================
-- Las tablas de WhatsApp están en archivo separado:
-- init_ms_auth_whatsapp.sql
-- Ejecutar después de este archivo si necesitas WhatsApp.
...
-- SELECT * FROM obtener_stats_emails_cuenta('uuid-cuenta-here');
--
...
-- WHERE cuenta_gmail_id = 'uuid-here' AND fecha_recibido < '2023-01-01';
--
...
-- SELECT * FROM obtener_cuentas_gmail_usuario('uuid-usuario-here');
--
-- =====================================
-- 📋 NOTAS IMPORTANTES
-- =====================================
...
-- =====================================
-- Todos los IDs migrados de SERIAL/INTEGER → UUID:
-- - usuarios_principales.id
-- - cuentas_gmail_asociadas.id + usuario_principal_id
-- - emails_sincronizados.id + cuenta_gmail_id  
-- - sesiones_jwt.id + usuario_principal_id
-- - audit_eliminaciones.id + registro_id
-- - whatsapp_accounts.id + usuario_principal_id
-- - conversations.id + whatsapp_account_id
-- - messages.id + conversation_id + whatsapp_account_id
-- - Todas las funciones actualizadas para usar UUID
-- =====================================