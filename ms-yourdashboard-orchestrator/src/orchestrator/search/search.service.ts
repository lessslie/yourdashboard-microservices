...
export class SearchService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.msCalendarUrl =
      ...
      'http://localhost:3005';
    this.msWhatsappUrl =
      this.configService.get<string>('MS_WHATSAPP_URL') ||
      'http://localhost:3004';
  }
  ...
  async searchGlobal(
    ...
    console.log('🔍 SearchService - Params recibidos:', {
    ...
    const [emailsResult, calendarResult, whatsappResult] =
    ...
    console.log(
      '[SEARCH] Resultados de emails, calendar y whatsapp:',
      emailsResult,
      calendarResult,
      whatsappResult,
    );
    ...
    if (emailsResult.status === 'fulfilled' && emailsResult.value) {
      ...
      console.error(
        '[SEARCH] Error en emails (continuando con otros servicios):',
        emailsResult.status === 'rejected'
          ? emailsResult.reason
          : 'Unknown error',
      );
    }
    ...
    if (calendarResult.status === 'fulfilled' && calendarResult.value) {
      ...
      console.error(
        '[SEARCH] Error en calendar (continuando con otros servicios):',
    ...
    if (whatsappResult.status === 'fulfilled' && whatsappResult.value) {
      ...
      console.error(
        '[SEARCH] Error en whatsapp (continuando con otros servicios):',
  ...
  private async extractUserIdFromToken(authHeader: string): Promise<string> {
    try {
      ...
      const msAuthUrl =
        this.configService.get<string>('MS_AUTH_URL') ||
        'http://localhost:3001';
      ...
      const response: AxiosResponse<AuthProfileResponse> = await axios.get(
        url,
        {
          headers: {
   ...
   */
  private async searchInEmails(
    userId: string,
    query: string,
    page: string,
    limit: string,
  ): Promise<EmailSearchResponse | null> {
    try {
      ...
      const response: AxiosResponse<EmailSearchResponse> = await axios.get(
        url,
        { params },
      );
   ...
   */
  private async searchInCalendar(
    userId: string,
    query: string,
    page: string,
    limit: string,
  ): Promise<CalendarSearchResponse | null> {
    try {
      ...
      const response: AxiosResponse<CalendarSearchResponse> = await axios.get(
        url,
        { params },
      );
  ...
  private async searchInWhatsapp(
    ...
    try {
      ...
      const response: AxiosResponse<WhatsappConversationRaw[]> =
      ...
      return {
        results: response.data.map(
          (conversation: WhatsappConversationRaw): WhatsappResult => ({
