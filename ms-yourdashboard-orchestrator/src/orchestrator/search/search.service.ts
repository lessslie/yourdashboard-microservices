// ms-yourdashboard-orchestrator/src/orchestrator/search/search.service.ts
// ✅ MIGRADO COMPLETO: SERIAL → UUID (number → string)
...
export class SearchService {
  ...
  async searchGlobal(authHeader: string, query: string, page: string, limit: string): Promise<GlobalSearchResponse> {
    ...
    // ✅ EXTRAER userId DEL TOKEN JWT (AHORA RETORNA STRING UUID)
  ...
  /**
   * ✅ MÉTODO CORREGIDO: Extraer userId del token JWT (retorna UUID string)
   */
  private async extractUserIdFromToken(authHeader: string): Promise<string> {
    try {
      ...
      // ✅ VALIDAR QUE EL ID ES STRING (UUID) Y NO CONVERTIR A STRING
      if (response.data?.usuario?.id && typeof response.data.usuario.id === 'string') {
        return response.data.usuario.id; // Ya es string UUID
      }

      throw new Error('No se pudo obtener el userId del token o no es UUID válido');
    } catch (error) {
  ...
  private async searchInWhatsapp(
    userId: string, // ✅ Ya es string (UUID)
    query: string,
    page: string,
    limit: string
    ...
    try {
      ...
      const params = {
        q: query,
        userId: userId // ✅ Ya es string (UUID)
      };
