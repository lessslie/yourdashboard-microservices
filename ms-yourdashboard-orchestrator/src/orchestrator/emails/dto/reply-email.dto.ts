...
export class ReplyEmailDto {
  @ApiProperty({
    ...
    example: 'Gracias por tu mensaje. Te respondo que estamos de acuerdo con la propuesta.',
    maxLength: 10000
  })
  @IsNotEmpty({ message: 'El contenido de la respuesta es requerido' })
  @IsString({ message: 'El contenido debe ser un texto válido' })
  @MaxLength(10000, { message: 'El contenido no puede exceder 10,000 caracteres' })
  body: string;

  @ApiProperty({
    description: 'Contenido de la respuesta en HTML (opcional)',
    example: '<p>Gracias por tu mensaje.</p><p>Te respondo que estamos de acuerdo con la propuesta.</p>',
    required: false,
    maxLength: 20000
  })
  @IsOptional()
  @IsString({ message: 'El contenido HTML debe ser un texto válido' })
  @MaxLength(20000, { message: 'El contenido HTML no puede exceder 20,000 caracteres' })
  bodyHtml?: string;
}
